opcoes_legiveis = {
    #Emoções
    'feliz': 'Feliz',
    'triste': 'Triste',
    'alteracao_humor': 'Alteração de humor',
    'sensivel': 'Sensível',
    'raiva': 'Raiva',
    'irritavel': 'Irritável',
    'ansiosa': 'Ansiosa',
    'falta_controle': 'Falta de controle',
    'indiferenca': 'Indiferença',

    #Mente
    'mente_confusa': 'Mente confusa',
    'calma': 'Calma',
    'estresse': 'Estresse',
    'motivacao': 'Motivação',
    'criatividade': 'Criatividade',
    'bom_rendimento': 'Bom rendimento',
    'preguica_desanimo': 'Preguiça ou desânimo',

    #Sociabilidade
    'sociavel': 'Sociável',
    'introvertida': 'Introvertida',
    'compreensiva': 'Compreensiva',
    'amorosa': 'Amorosa',
    'conflituosa': 'Conflituosa',

    #Lazer
    'ferias': 'Férias',
    'encontros': 'Encontro',
    'ressaca': 'Ressaca',
    'alcool': 'Álcool',
    'cigarro': 'Cigarro',

    #Sintomas físicos
    'dor_cabeca': 'Dor de cabeça',
    'surto_acne': 'Surto de acne',
    'tensao_corporal': 'Tensão corporal',
    'dor_corporal': 'Dor muscular','alergia_dermatite': 'Alergia/Dermatite',
    'gripe': 'Gripe ou doença',
    'insonia': 'Insônia',
    'alteracao_hormonal': 'Alterações hormonais',
    'problemas_digestivos': 'Problemas digestivos', 
    'sem_apetite': 'Sem apetite',       
    'queda_cabelo': 'Queda de cabelo',
    'taquicardia': 'Taquicardia',

    #Conversa
    'palavras_carinho': 'Palavras de carinho',
    #'presentes':'Presentes',
    'xingamentos': 'Xingamentos',       
    'piadas_ofensivas': 'Piadas ofensivas',
    'mentira': 'Mentiras',
    'dar_gelo': 'Dar um gelo',
    'ciumes': 'Ciúmes',   
    'culpar': 'Culpar',     'proibir': 'Proibir',    
    'chantagem': 'Chantagear',
    
    #Comportamentos
    'empurrar': 'Empurrar',  
    'beliscar': 'Beliscar ou arranhar',
    'chutar': 'Chutar',
    'apertar': 'Apertar', 
    'bater': 'Bater',  
    'destruir_bens': 'Destruir bens',
    'brincar_bater': 'Brincar de bater',    
    'obrigou_relacao_sexual': 'Obrigou a ter relações',
    'abuso_sexual': 'Abuso sexual',
    'confinar': 'Confinar ou prender',
    'sufocar': 'Sufocar ou estrangular',

    #Socialização
    'feriu_animal': 'Matou ou feriu animal',    'tentou_se_matar': 'Tentou se matar',
    #'empurrar_outro':'Empurrar',
    #'beliscar_outro': 'Beliscar ou arranhar',
    #'chutar_outro': 'Chutar',
    #'bater_outro': 'Bater',
    'ameacar': 'Ameaçar',
    #'apertar_outro': 'Apertar'
}

categorias_legiveis = {
    'emocoes': 'Emoções',
    'mente': 'Mente',
    'sociabilidade': 'Sociabilidade',
    'lazer': 'Lazer',
    'sintomas': 'Sintomas físicos',
    'conversa': 'Conversa',
    'comportamentos': 'Comportamentos',
    'socializacao': 'Socialização'
}
